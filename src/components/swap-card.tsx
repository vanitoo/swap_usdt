...
export function SwapCard() {
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-glow backdrop-blur md:p-7">
      ...
      <div className="space-y-4">
        <label className="block">
          ...
          <select
            ...
            className="w-full rounded-2xl border border-white/10 bg-[#070b12] px-4 py-4 text-white outline-none focus:border-emerald-300"
          >
        ...
        <label className="block">
          ...
          <div className="flex rounded-2xl border border-white/10 bg-[#070b12] focus-within:border-emerald-300">
            <input
              value={amount}
              onChange={(event) => setAmount(event.target.value)}
              inputMode="decimal"
              className="min-w-0 flex-1 bg-transparent px-4 py-4 text-2xl text-white outline-none placeholder:text-white/30"
              placeholder="0.00"
            />
        ...
        <div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/10 p-4">
          ...
          <div className="mt-1 text-3xl font-bold">
            {estimated} {direction.to}
          </div>
          <div className="mt-2 text-xs text-white/50">
            Сейчас это демо-расчет. Реальный обмен подключим следующим этапом.
          </div>
        </div>

        <button
          type="button"
          className="w-full rounded-2xl bg-emerald-300 px-5 py-4 font-bold text-black transition hover:bg-emerald-200"
          onClick={() => alert("Подключение кошелька и реальный swap подключим следующим этапом.")}
        >
          Подключить кошелек
        </button>

        <p className="text-center text-xs text-white/45">
          Сервис не хранит средства. Все операции будут подтверждаться в кошельке пользователя.
        </p>
      </div>
    </section>
  );
